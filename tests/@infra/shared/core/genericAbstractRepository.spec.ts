...
import { GenericAbstractRepository, type IConnectionContract, type IMapperContract } from '@infra/shared/'

describe('GenericAbstractRepository', () => {
  // Define an ORM type
  const ORM = {
    findOne: () => 1,
    findMany: () => [1, 2, 3],
    ...
    //
    // Mock connection
    const connection: IConnectionContract<any, typeORM> = {
      delete: jest.fn(),
      find: jest.fn(),
      save: jest.fn(),
      orm: () => ORM
    ...
    // Mapper mock
    const mapper: IMapperContract<any, any> = {
      toDomain: jest.fn(),
